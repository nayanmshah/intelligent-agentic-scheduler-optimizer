...
| **NFR-02** | **Time-to-offer < 5 s p95 with a live LLM call** | Same. **Beyond ~5 s the human opens the calendar anyway and the value proposition is lost — this is a functional threshold, not a nicety** |
...
| ID | Requirement | Acceptance |
| :- | :---------- | :--------- |
| **NFR-09** | **Full offline operation.** Every MUST requirement works with no network access | **Verified end to end with networking disabled.** Live LLM access is a single explicit opt-in, never implicit |
...
| **R-13** | **The explainer may not need to be an LLM at all — a conceded weakness.** | Templates get roughly 90% of the value. The LLM buys naturalness and avoids combinatorial template explosion as the number of rationale combinations grows. The template is always computed and always available as the fallback, so the LLM can be removed from this stage at any time without loss of function | FR-060 (both renderings returned); NFR-28 |
| **R-14** | **Network or LLM API unavailable at runtime.** | Committed fixtures are the **default**, not the fallback; live LLM access is an explicit opt-in. Every MUST requirement is satisfied with networking disabled, and the eval harness reports rules-mode accuracy so the offline path's quality is a known number rather than a hope | NFR-09; FR-005, FR-006, FR-105 |
